    ...
    private void Update()
    {
        ...
        if (jigPos)
            transform.localPosition =
             basePosition + positionJigAmount * Mathf.Sin(positionTime) * power;
        ...
        if (jigRot)
            transform.localRotation =
                baseRotation * Quaternion.Euler(rotationJigAmount *
                                        Mathf.Sin(positionTime) * power);

        scaleTime += dt * scaleFrequency;

        if (jigScale)
            transform.localScale =
                baseScale + scaleJigAmount * Mathf.Sin(scaleTime) * power;
    }
}
